...
def movie_api_get():
    ...
    df=pd.read_json(response, orient='split').reset_index().rename(columns={'index':'movie_id', 'customerID':'user_id', 'current_time':'timestamp'})
    df['user_rating'] = 4
    df = df[['user_id', 'movie_id', 'user_rating', 'timestamp']]
    return df
...
try:
    while True:
        data = movie_api_get()
        current_time = data['timestamp'][0]
        print(f"Producing record - time: {current_time}")
        record_value = data.to_json()
        # This will actually send data to your topic
        producer.produce(
            TOPIC,
            key=f"netflix_{current_time}",
            value=record_value,
            on_delivery=acked
        )
